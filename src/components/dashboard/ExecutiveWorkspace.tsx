...
export default function ExecutiveWorkspace() {
  ...
  const [timeframe, setTimeframe] = useState<'7D' | '14D' | 'ALL'>('14D'); // Matches active 14D in screenshot
  ...
  if (!data) return <div className="p-12 font-mono text-xs text-zinc-500 text-center animate-pulse">Syncing Core Telemetry Planes...</div>;
  ...
  const spendLinePath = data.points.map((p, i) => {
    ...
    const y = 140 - (Math.min(p.spend, maxSpend) / maxSpend) * 110; // Bounds matched inside layout frame
    return `${x},${y}`;
  }).join(' ');
  ...
  return (
    <div className="w-full min-h-screen bg-[#03060e] text-zinc-100 p-6 space-y-6 select-none font-sans">
      
      {/* --- TOP BANNER 4-COLUMN DATA CARD MATRIX --- */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        
        {/* Card 1: Total Period Spend */}
        <div className="p-5 bg-[#0b0f17] border border-zinc-900/80 rounded-xl relative">
          ...
          <p className="text-[10px] text-zinc-500 font-mono mt-1">14D Aggregation</p>
        </div>
          ...
          <div className="relative w-full h-64 mt-4 flex">
            ...
            <div className="flex-1 h-full relative border-b border-zinc-800">
              <svg className="w-full h-full overflow-visible" viewBox="0 0 500 150" preserveAspectRatio="none">
                ...
                <polyline fill="none" stroke="#52e1a2" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" points={spendLinePath} />

                {/* Path 2: Carbon Intensity Vector (Dashed Soft-Green line) */}
                <polyline fill="none" stroke="#10b981" strokeWidth="1.5" strokeDasharray="4" strokeLinecap="round" strokeLinejoin="round" points={carbonLinePath} />
        ...
        <div className="lg:col-span-3 space-y-4 flex flex-col justify-between">
          ...
          <div className="p-5 bg-[#0b0f17] border border-zinc-900/80 rounded-2xl space-y-4 flex-1">
            ...
            {data.insights.map((insight, idx) => (
              <div key={idx} className="p-3 bg-[#03060e] border border-zinc-900/60 rounded-xl text-xs text-zinc-300 leading-relaxed flex gap-2">
          ...
          <div className="p-5 bg-[#0b0f17] border border-zinc-900/80 rounded-2xl flex items-center gap-4">
